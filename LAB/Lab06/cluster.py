...
class KMeans:
    ...
    def single_fit_predict(self, X: np.ndarray):
        ...
        while True:  # stopping criterion
            ...
            Update the centers.
            """
            centers = np.array([X[new_assignments == k].mean(axis=0) if np.any(new_assignments == k) else centers[k]
                                for k in range(self.n_cl)])

            """
            Check the break condition.
            """
            if np.all(old_assignments == new_assignments): # se non sono cambiati gli assignment
                break
    ...
    def compute_cost_function(self, X: np.ndarray, centers: np.ndarray,
        ...
        cost = np.sum(np.linalg.norm(X - centers[assignments], axis=1) ** 2)
        return cost
