...
class SVM:
    ...
    def loss(self, y_true, y_pred):
        ...
        """
        return 0.5 * self._lambda * norm(self._w) + np.mean(
            np.maximum(
                np.zeros(
                    y_true.shape[0],
                ),
    ...
    def fit_gd(self, X, Y, verbose=False):
        ...
        for e in range(1, self._n_epochs + 1):
            for j in range(n_samples):
                ...
                if y_j * X_j_pred < 1:
                    self._w = (1.0 - n_t * self._lambda) * self._w + (X_j * y_j * n_t)
                else:
    ...
    def predict(self, X):
        ...
        """
        return np.where(
            np.dot(X, self._w) > 0.0, self._original_labels[1], self._original_labels[0]
        )
